...
PlayGame::PlayGame(TileGrid &tg, bool (*allMatch)(Grid<Tile> &), bool &donePlayingManually) {
    mouseDown = false;
    currIm = nullptr;
...
void PlayGame::playGameManually(TileGrid &tg, bool &donePlayingManually) {
    ...
    tg.getWindow().setMouseListener([&tg,&donePlayingManually,this](GEvent e) {
...
bool PlayGame::handleMouseEvent(TileGrid &tg, GMouseEvent e) {
    ...
    if (etype == MOUSE_PRESSED) {
        ...
        if (allMatch(tiles)) {
            ...
            GOptionPane::ConfirmResult res = GOptionPane::showConfirmDialog("You solved the game!\n"
